...
class GeneralizedResNet_imagenet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if 'depth_resnet' in args:
            if args['depth_resnet']: 
                d = args['depth_resnet']
                if d == 10:
                    layers = [1,1,1,1]
                elif d == 18:
                    ...
                    assert 0 # depth_resnet is invalid

        self.init_width = 64
        if 'width_resnet' in args:
            if args['width_resnet']:
                self.init_width= args['width_resnet']

        self.final_size = 1
        if 'final_size' in args:
            if args['final_size']:
                self.final_size= args['final_size']

        self.inplanes = self.init_width
        self.dilation = 1
        ...
        self.layer1 = self._make_layer(block, self.init_width, layers[0])
        self.layer2 = self._make_layer(block, self.init_width*2, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        if d==10:
            self.layer3 = None
        else:
            self.layer3 = self._make_layer(block, self.init_width*4, layers[2], stride=2,
...
class SpecializedResNet_imagenet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.feature_dim = self.init_width*8 * block.expansion *self.final_size*self.final_size
        if d == 10:
            self.inplanes = self.init_width*2 * block.expansion
        else:
            self.inplanes = self.init_width*4 * block.expansion
        self.dilation = 1
        if replace_stride_with_dilation is None:
            replace_stride_with_dilation = [False, False, False]
        if len(replace_stride_with_dilation) != 3:
        ...
        self.layer4 = self._make_layer(block, self.init_width*8, layers[3], stride=2,
                                        dilate=replace_stride_with_dilation[2])
        
        self.avgpool = nn.AdaptiveAvgPool2d((self.final_size, self.final_size))
        self.out_dim = self.init_width*8 * block.expansion *self.final_size*self.final_size
...
def get_resnet_scale(args=None):
    basenet = GeneralizedResNet_imagenet(BasicBlock,[2, 2, 2, 2],args=args)
    adaptivenet = SpecializedResNet_imagenet(BasicBlock, [2, 2, 2, 2],args=args)
    return basenet,adaptivenet
